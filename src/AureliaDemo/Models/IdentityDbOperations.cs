namespace AureliaDemo.Models
{
    ...
    {
        private static readonly Logger Logger = Logger.GetLogger(typeof(IdentityDbOperations).Name);
        ...
        public static async Task InitializeIdentityDbAsync(IServiceProvider serviceProvider)
        {
            ...
            {
                var relationalDatabase = db.Database as RelationalDatabase;

                if (relationalDatabase != null)
                {
                    // Create the database if it does not already exist
                    Logger.Trace("InitializeIdentityDbAsync: Ensuring Database exists");
                    await relationalDatabase.EnsureCreatedAsync();
